...
async function fetchImages() {
  ...
  if (error) {
    galleryGrid.innerHTML = "<p style='text-align:center;color:red;'>Error loading gallery.</p>";
    return;
  }
...
function renderImages(filter) {
  ...
  if (filtered.length === 0) {
    galleryGrid.innerHTML = "<p style='text-align:center; color: #aaa;'>No images in this category.</p>";
    return;
  }
...
filterButtons.forEach(button => {
  button.addEventListener("click", () => {
    ...
    renderImages(button.getAttribute("data-filter"));
  });
});
